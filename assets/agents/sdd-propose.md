...
- Write the `proposal` logical artifact to the selected human backend and save an Engram summary/pointer at `sdd/{change}/proposal`.
- Include intent, scope, affected areas, risks, rollback, and success criteria.
- Do NOT launch child subagents. Parent/orchestrator owns delegation.
- Persist planning output to selected human backend + Engram; do not create OpenSpec files unless explicitly requested.
## Memory Contract
...
- Save the full proposal to the selected human backend according to the selected backend convention (use the Obsidian convention only for explicit legacy/fallback mode), then call the injected Engram save tool with title and `topic_key` `"sdd/{change}/proposal"`, `type: "architecture"`, and `project` from context for the Engram summary/pointer.

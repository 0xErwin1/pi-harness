...
- Save project context, strict TDD mode, phase rules, and testing runner details to selected human backend + Engram (`topic_key: sdd-init/{project}`).
...
- Save the full project context to the selected human backend according to the selected backend convention (use the Obsidian convention only for explicit legacy/fallback mode), then call the injected Engram save tool with title and `topic_key` `"sdd-init/{project}"`, `type: "architecture"`, and `project` from context for the Engram summary/pointer.

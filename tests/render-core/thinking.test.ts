...
test("renderThinkingBlock: plain body produces an italic thinking-tinted header + muted body", () => {
	...
	assert.ok(result[0].includes("[italic:"), `header must be italicised: ${result[0]}`);
	assert.ok(result[0].includes("[thinking:"), `header must use the thinking tint: ${result[0]}`);
	assert.ok(result[0].includes("Thinking"), `header should contain 'Thinking': ${result[0]}`);
	assert.ok(result.slice(1).every(l => l.includes("[muted:")), `body lines must be muted: ${JSON.stringify(result.slice(1))}`);
...
test("renderThinkingBlock: bold-prefixed text lifts title into a bold header", () => {
	...
	assert.ok(result[0].includes("[bold:Planning]"), `title must be bold: ${result[0]}`);
...
test("renderThinkingBlock: body lines are flush (no gutter), matching opencode", () => {
	...
	assert.ok(bodyLines.every(l => !l.startsWith("│")), `body lines must not carry a gutter: ${JSON.stringify(bodyLines)}`);
});

test("renderThinkingBlock: a long unbroken body wraps to width instead of truncating", () => {
	const width = 15;
	const long = `${"word ".repeat(20).trim()}`;
	const result = renderThinkingBlock([long], makeCtx(width, PLAIN));

	assert.ok(result.every(l => l.length <= width), `every line must fit ${width}: ${JSON.stringify(result)}`);
	assert.ok(result.slice(1).length > 1, "a long body must wrap into multiple lines, not truncate to one");
	assert.ok(result.join(" ").includes("word"), "the body text must survive wrapping");
});

test("renderThinkingBlock: header uses the thinking tint, body uses muted", () => {
	const colors: string[] = [];
	...
	assert.equal(colors[0], "thinking", `header must use the thinking tint, got: ${JSON.stringify(colors)}`);
	assert.ok(colors.slice(1).every(c => c === "muted"), `body must use muted, got: ${JSON.stringify(colors)}`);
});
...
test("renderThinkingBlock: blank body lines are skipped", () => {
	...
	const bodyLines = result.slice(1);
	assert.ok(!bodyLines.some(l => l.trim() === ""), `blank body lines should be omitted`);
});

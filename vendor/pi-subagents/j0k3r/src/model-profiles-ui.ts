...
import { loadSubagents, readSubagentsConfig, resetSubagentModelProfileField, saveSubagentModelProfile } from './config.ts';
import { writeAgentFrontmatterProfile } from './agent-frontmatter.ts';
import { resolveEffectiveSubagentProfile } from './profile-resolver.ts';
import type { ModelRef, SubagentDefinition, SubagentDefinitionScope, SubagentModelProfile, SubagentModelProfiles, SubagentsConfig, ThinkingEffort } from './types.ts';
...
export type ModelProfilePersistence = {
  mode: 'frontmatter' | 'json';
  filePath?: string;
...
export type ModelProfileRow = {
  ...
  scope?: SubagentDefinitionScope;
  persistence: ModelProfilePersistence;
};

function persistenceForDefinition(definition: SubagentDefinition): ModelProfilePersistence {
  if (definition.filePath && definition.scope !== 'project') {
    return { mode: 'frontmatter', filePath: definition.filePath, scope: 'global' };
  }
  return { mode: 'json', scope: 'global' };
}

function explicitProfileForPersistence(definition: SubagentDefinition, config: SubagentsConfig, persistence: ModelProfilePersistence): SubagentModelProfile {
  if (persistence.mode === 'frontmatter') {
    return {
      ...(definition.model ? { model: definition.model } : {}),
      ...(definition.effort ? { effort: definition.effort } : {}),
    };
  }
  return { ...(config.global_model_profiles?.[definition.name] ?? {}) };
}
...
export function buildModelProfileRows(input: {
  ...
  return [...byName.values()]
    ...
    .map((definition) => {
      ...
      const persistence = persistenceForDefinition(definition);
      const explicitProfile = explicitProfileForPersistence(definition, input.config, persistence);
      return {
...
export function commitStagedModelProfiles(input: { stagedProfiles: SubagentModelProfiles; save: boolean; agentDir?: string; cwd?: string; persistenceTargets?: Record<string, ModelProfilePersistence> }): string {
  ...
  if (!input.save) return 'Cancelled. No changes were saved.';
  const touched = new Set<string>();
  for (const [agentName, profile] of Object.entries(input.stagedProfiles)) {
    try {
      const target = input.persistenceTargets?.[agentName.trim().toLowerCase()] ?? { mode: 'json' as const, scope: 'global' as const };
      if (target.mode === 'frontmatter' && target.filePath && target.scope !== 'project') {
        touched.add(writeAgentFrontmatterProfile({ filePath: target.filePath, model: profile.model, thinking: profile.effort }));
        continue;
      }
      touched.add(globalPath);
      if (profile.model || profile.effort) saveSubagentModelProfile({ agentName, profile, scope: 'global', cwd: input.cwd, agentDir: input.agentDir });
      else {
        resetSubagentModelProfileField({ agentName, field: 'model', scope: 'global', cwd: input.cwd, agentDir: input.agentDir });
        resetSubagentModelProfileField({ agentName, field: 'effort', scope: 'global', cwd: input.cwd, agentDir: input.agentDir });
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`Error saving model/thinking assignment for ${agentName}: ${message}`);
    }
  }
  const targets = [...touched];
  return targets.length > 1
    ? `Saved model assignments to ${targets.join(' and ')}.`
    : `Saved model assignments to ${targets[0] ?? globalPath}.`;
}

export function buildNoChangesModelProfilesMessage(): string {
  return 'No subagent model or thinking changes to save.';
}
...
export function createSubagentModelProfilesModal(input: ModalInput): ModalComponent {
  ...
  const renderMain = (width: number): string[] => {
    ...
    const body = [
      `target: global assignments; global agent files keep frontmatter · ${pendingLabel(dirtyCount)}`,
...
export function buildNonTuiModelProfilesMessage(agentDir?: string): string {
  return `subagent model profiles require Pi TUI. Edit subagent markdown frontmatter or JSON profiles manually in ${globalSubagentsConfigPath(agentDir)}.`;
}
...
async function chooseSave(ctx: any, stagedProfiles: SubagentModelProfiles, input: { agentDir?: string; cwd?: string; persistenceTargets?: Record<string, ModelProfilePersistence> } = {}): Promise<string> {
  const decision = await ctx.ui.select('Save subagent model profile changes?', ['Save', 'Cancel']);
  const message = commitStagedModelProfiles({ stagedProfiles, save: decision === 'Save', agentDir: input.agentDir, cwd: input.cwd, persistenceTargets: input.persistenceTargets });
...
export async function runSubagentModelsCommand(ctx: any = {}): Promise<string> {
  ...
  const persistenceTargets: Record<string, ModelProfilePersistence> = Object.fromEntries(rows.map((row) => [row.name.trim().toLowerCase(), row.persistence]));
  ...
  if (hasCustomUi) {
    ...
    if (result?.action === 'save') {
      ...
      const message = hasDirtyRows
        ? commitStagedModelProfiles({ stagedProfiles: result.dirtyProfiles, save: true, agentDir, cwd, persistenceTargets })
  ...
  return chooseSave(ctx, staged, { agentDir, cwd, persistenceTargets });
}

import type { EffectiveSubagentProfile, ModelRef, ProfileValueSource, ResolvedProfileField, SubagentDefinition, SubagentsConfig, ThinkingEffort } from './types.ts';
...
function profileForDefinition(definition: SubagentDefinition, config: SubagentsConfig) {
  if (definition.scope === 'project') return config.global_model_profiles?.[definition.name] ?? (config.global_model_profiles ? undefined : config.model_profiles[definition.name]);
  if (definition.filePath) return undefined;

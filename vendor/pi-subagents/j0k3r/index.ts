...
import { readSubagentsConfig, subagentSourceWarnings } from './src/config.ts';
import { registerSubagentTools, triggerClaudeBackgroundHandoff } from './src/tools.ts';
import { runSubagentModelsCommand } from './src/model-profiles-ui.ts';
import { SubagentsHistoryPanel } from './src/ui.ts';
...
export default function subagentsExtension(pi: any): void {
  ...
  const installClaudeBackgroundWidget = (ctx: any): boolean => {
    ...
    let cachedTasks = manager.listSessionTasks(cwd, sessionId).slice(0, 100);
    widgetState = new ClaudeBackgroundWidgetState(
      () => cachedTasks,
      () => widgetRequestRender?.(),
    );
    const refreshCachedTasks = () => {
      const previousActive = cachedTasks.some(isActiveBackgroundTask);
      cachedTasks = manager.listSessionTasks(cwd, sessionId).slice(0, 100);

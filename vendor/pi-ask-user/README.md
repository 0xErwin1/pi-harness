...
## Harness vendoring note

This directory vendors `https://github.com/edlsh/pi-ask-user` and keeps the upstream implementation in `upstream.ts`. The harness entrypoint in `index.ts` re-exports the upstream extension, so the registered tool is `ask_user`.
